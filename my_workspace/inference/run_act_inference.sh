...
CAMERA_LAPTOP_INDEX=0  # 或使用绝对路径 "/dev/video0"
CAMERA_PHONE_INDEX=2   # 或使用绝对路径 "/dev/video2"
...
# ACT模型检查点路径
MODEL_PATH="${PROJECT_ROOT}/output/act_train_20251003_174818_1452/checkpoints/152000/pretrained_model"
...
# 评估数据保存路径
EVAL_DATA_ROOT="${PROJECT_ROOT}/eval_data/act_eval_$(date +%Y%m%d_%H%M%S)"

# 推理参数
NUM_EPISODES=5          # 运行的episode数量
EPISODE_TIME_S=30       # 每个episode的时长（秒）
CONTROL_FPS=30          # 控制频率（Hz）

# 任务描述（可选，某些策略需要）
TASK_DESCRIPTION="Grasp and place object"

# 安全参数
MAX_RELATIVE_TARGET=10.0  # 限制单步最大移动角度（度），防止突然大幅度运动

# 可视化
DISPLAY_DATA=true  # 启用Rerun实时可视化
